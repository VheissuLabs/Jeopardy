...
- **MUST** break method chains of three or more calls: every link on its own line. Two-call chains (`$request->session()->get(...)`) may stay inline; a multi-line chain must never carry two links on one line. (Enforced by `tests/Architecture/ConventionsTest.php`.)
- **MUST** write associative array literals multi-line, one `key => value` per line — even single-entry ones (`->update(['status' => ...])`). Plain lists (`['required', 'string']`) may stay inline. (Enforced by `tests/Architecture/ConventionsTest.php`.)
- **MUST** write a docblock whose only content is a single tag on one line — `/** @return array<string, mixed> */` — unless the collapsed line would exceed 120 characters. Docblocks with a description stay multi-line. (Enforced by `tests/Architecture/ConventionsTest.php`.)
