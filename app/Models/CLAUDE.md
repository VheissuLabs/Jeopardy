...
- **MUST** write a docblock whose only content is a single tag as one line — `/** @return BelongsTo<User, $this> */`, `/** @mixin IdeHelperBoard */`. Docblocks with a description stay multi-line. (Enforced by `tests/Architecture/ConventionsTest.php`.)
